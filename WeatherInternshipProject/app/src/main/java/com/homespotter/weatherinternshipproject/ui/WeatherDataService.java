...
public class WeatherDataService extends IntentService {
    ...
    private Thread currentConditionsThread;
    private Thread threeHoursForecastThread;
    private Thread dailyForecastThread;
        ...
        return new Runnable() {
            ...
            public void run() {
                ...
                threeHoursForecast = DataParser.parseThreeHourForecast(data);
                    ...
                    Log.d(TAG, "threeHoursForecast", e);
    ...
    private synchronized void onThreadDone(int result) {
        Log.d(TAG, "onThreadDone " + result);

        if (result != SERVICE_RESULT_OK) {
            // finish and return error
            currentConditionsThread.interrupt();
            threeHoursForecastThread.interrupt();
            dailyForecastThread.interrupt();
    ...
    protected void onHandleIntent(Intent workIntent) {
        ...
        if (checkInternetAccess()) {
            currentConditionsThread = new Thread(getFetchCurrentConditions());
            currentConditionsThread.start();
            threeHoursForecastThread = new Thread(getFetchThreeHoursForecast());
            threeHoursForecastThread.start();
            dailyForecastThread = new Thread(getFetchDailyForecast());
            dailyForecastThread.start();
        }
        else {
